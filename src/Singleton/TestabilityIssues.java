...
class SingletonDatabase
{
    ...
    {
        return INSTANCE; //classic
    }

    public int getPopulation(String name) //some utility method
    {
...
class SingletonRecordFinder
{
    public int getTotalPopulation(List<String> names)
    {
        ...
        for (String name : names)
            result += SingletonDatabase.getInstance().getPopulation(name); //testability problems
        return result;
    }
}
...
class ConfigurableRecordFinder
{
    ...
    public int getTotalPopulation(List<String> names)
    {
        int result = 0;
        for (String name : names)
            result += database.getPopulation(name); // hard dependency disables unit test usage => add abstraction
        return result;
    }
}
...
class DummyDatabase implements Database
{
    ...
    public DummyDatabase() { //now we can create a unit test
        data.put("alpha", 1);
        data.put("beta", 2);
        data.put("gamma", 3);
    }
